...
class AIModeration:
    def __init__(self, bot: commands.Bot):
        ...
        try:
            ...
            self.enabled = True
            logging.info(f"✅ AI Moderation module initialized with {model_name}")
        except Exception as e:
            logging.error(f"❌ Failed to initialize Gemini model: {e}")
            logging.error("⚠️ AI moderation will be disabled")
            self.enabled = False
  ...
  "suggested_action": "warn/timeout/kick/ban (or null if no violation)"
    ...
    async def handle_violation(
        ...
        try:
            ...
            # Try to delete the offending message (SILENT MODE - no embed to save tokens)
            try:
                await message.delete()
                logging.info(
                    f"🤖 AI Auto-moderation: Deleted message from {message.author} in {message.guild.name} "
                    f"for: {rule_broken}"
                )
            except discord.Forbidden:
                logging.warning(f"⚠️ Could not delete message from {message.author} - missing permissions")
