...
# Try to import AI moderation, but don't crash if dependencies missing
try:
    from ai_moderation import setup as setup_ai_moderation
    AI_MODERATION_AVAILABLE = True
except ImportError as e:
    logging.warning(f"⚠️ AI Moderation not available: {e}")
    logging.warning("⚠️ Install google-generativeai: pip install google-generativeai")
    AI_MODERATION_AVAILABLE = False
    setup_ai_moderation = None
...
# Initialize AI Moderation (if available)
ai_mod = setup_ai_moderation(bot) if AI_MODERATION_AVAILABLE else None
if ai_mod:
    logging.info("✅ AI Moderation enabled")
else:
    logging.info("⚠️ AI Moderation disabled (missing dependencies)")
...
async def on_error(event, *args, **kwargs):
    ...
    if exc_type == discord.HTTPException:
        if exc_value.status == 429:
            logging.warning(f"⚠️ Rate limited in event: {event}")
            return
    logging.error(f"Error in {event}: {exc_value}")
    traceback.print_exc()
...
@tree.command(name="toggleai", description="🤖 Enable or disable AI-powered moderation")
@app_commands.describe(
    status="Turn AI moderation on or off"
)
@app_commands.choices(status=[
    app_commands.Choice(name="On - Enable AI Moderation", value="on"),
    app_commands.Choice(name="Off - Disable AI Moderation", value="off")
])
@app_commands.checks.has_permissions(manage_guild=True)
async def toggleai(
    interaction: discord.Interaction,
    ...
    """Toggle AI moderation on/off for the server"""
    await interaction.response.defer(ephemeral=True)
    
    # Check if AI is available
    if not ai_mod.enabled:
        emb = discord.Embed(
            title="❌ AI Moderation Unavailable",
            description="AI moderation is not configured. Please contact the bot owner to set up `GEMINI_API_KEY`.",
            color=Colors.DANGER
        )
        emb.set_footer(text="Custos AI Moderation")
        await interaction.followup.send(embed=emb, ephemeral=True)
        return
    
    # Parse status - choices decorator already converts to value string
    enable = (status.lower() == 'on')
    
    guild_id = interaction.guild.id
    
    # Check if rules have been analyzed
    if enable:
        guild_config = ai_mod.server_ai_config.get(guild_id, {})
        if not guild_config.get("analyzed", False):
            # Analyze rules first
            try:
                rules = await ai_mod.analyze_server_rules(interaction.guild)
                logging.info(f"🤖 Analyzed rules for {interaction.guild.name}")
            except Exception as e:
                logging.error(f"Error analyzing rules: {e}")
                emb = discord.Embed(
                    title="❌ Error",
                    description="Failed to analyze server rules. Please try again.",
                    color=Colors.DANGER
                )
                emb.set_footer(text="Custos AI Moderation")
                await interaction.followup.send(embed=emb, ephemeral=True)
    ...
    # Set AI status
    ai_mod.set_ai_enabled(guild_id, enable)
    
    # Create response embed
    if enable:
        emb = discord.Embed(
            title="✅ AI Moderation Enabled",
            description="🤖 AI-powered moderation is now **ACTIVE** in this server!",
            color=Colors.SUCCESS
        )
        emb.add_field(
            name="📜 Rules Analyzed",
            value="✓ Server rules have been analyzed",
            inline=False
        )
        emb.add_field(
            name="🔍 What It Does",
            value="• Monitors all messages in real-time\n• Automatically detects rule violations\n• Issues warnings to rule breakers\n• Integrates with existing warning system",
            inline=False
        )
        emb.add_field(
            name="⚠️ Note",
            value="AI moderation works alongside your existing moderation. Moderators can still manually warn/ban users.",
            inline=False
        )
    else:
        emb = discord.Embed(
            title="🔴 AI Moderation Disabled",
            description="AI-powered moderation has been turned off.",
            color=Colors.DANGER
        )
        emb.add_field(
            name="ℹ️ Manual Moderation",
            value="You can still use `/warn`, `/mute`, `/ban` commands manually.",
            inline=False
        )
    ...
    # Log to mod channel
    log_emb = discord.Embed(
        title="🤖 AI Moderation Status Changed",
        description=f"AI moderation has been **{'ENABLED' if enable else 'DISABLED'}**",
        color=Colors.SUCCESS if enable else Colors.DANGER,
        timestamp=datetime.now(timezone.utc)
    )
    log_emb.add_field(
        name="👮 Changed By",
    ...
    log_emb.set_footer(
        text="Custos AI Moderation System",
        icon_url="https://i.imgur.com/AfFp7pu.png"
    )
    await send_log(interaction.guild, log_emb)

@tree.command(name="aistatus", description="🤖 Check AI moderation status for this server")
@app_commands.checks.has_permissions(moderate_members=True)
async def aistatus(interaction: discord.Interaction):
    ...
    if not ai_mod.enabled:
        emb = discord.Embed(
            ...
            description="AI moderation system is not configured.",
            color=Colors.DANGER
        )
        emb.add_field(
            name="ℹ️ Information",
            value="The bot owner needs to configure `GEMINI_API_KEY`.",
            inline=False
        )
        await interaction.followup.send(embed=emb, ephemeral=True)
        return
    ...
    guild_id = interaction.guild.id
    is_enabled = ai_mod.is_ai_enabled(guild_id)
    guild_config = ai_mod.server_ai_config.get(guild_id, {})
    rules_analyzed = guild_config.get("analyzed", False)
    
    if is_enabled:
        emb = discord.Embed(
            title="✅ AI Moderation Active",
            description="🤖 AI moderation is currently **ENABLED** in this server",
            color=Colors.SUCCESS
        )
        emb.add_field(
            name="📊 Status",
            value="✅ Monitoring all messages",
            inline=True
        )
        emb.add_field(
            name="📜 Rules",
            value="✅ Analyzed" if rules_analyzed else "⚠️ Not analyzed",
            inline=True
        )
        emb.add_field(
            name="🔧 Actions",
            value="Use `/toggleai off` to disable",
            inline=False
        )
    else:
        emb = discord.Embed(
            title="🔴 AI Moderation Inactive",
            description="AI moderation is currently **DISABLED** in this server",
            color=Colors.DANGER
        )
        emb.add_field(
            name="📊 Status",
            value="❌ Not monitoring messages",
            inline=True
        )
        emb.add_field(
            name="📜 Rules",
            value="✅ Analyzed" if rules_analyzed else "⚠️ Not analyzed yet",
            inline=True
        )
        emb.add_field(
            name="🔧 Actions",
            value="Use `/toggleai on` to enable",
            inline=False
        )
...
async def on_ready():
    ...
    try:
        ...
        if e.status == 429:
            logging.warning(f"⚠️ Rate limited during sync, waiting 60s...")
            await asyncio.sleep(60)
            try:
                synced = await tree.sync()
                logging.info(f"✅ Synced {len(synced)} slash commands (retry)")
            except Exception as retry_e:
                logging.error(f"❌ Failed to sync after retry: {retry_e}")
        else:
    ...
    try:
        ...
        logging.warning("⚠️ Could not set presence (rate limited)")
    logging.info(f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
...
async def on_guild_join(guild):
    ...
    # AI: Analyze server rules automatically
    if ai_mod.enabled:
        try:
            rules = await ai_mod.analyze_server_rules(guild)
            logging.info(f"🤖 AI analyzed rules for {guild.name}")
        except Exception as e:
            logging.error(f"Error analyzing rules: {e}")
    ...
    welcome_emb.add_field(
        name="🤖 AI Moderation (NEW!)",
        value="• Use `/toggleai on` to enable AI-powered moderation\n• Bot will auto-detect rule violations\n• Powered by Google Gemini AI",
        inline=False
    )
    welcome_emb.add_field(
...
async def on_message(message):
    """Monitor messages for AI moderation"""
    # Ignore bot messages
    ...
    # Check if AI moderation is enabled for this server
    if not message.guild:
        return
    
    if not ai_mod.is_ai_enabled(message.guild.id):
        return
    
    # FIX #4: Check if message has content before checking length
    ...
    # Rate limiting: don't check same user too frequently
    if not ai_mod.should_check_message(message.guild.id, message.author.id):
        return
    
    # Get server rules
    guild_config = ai_mod.server_ai_config.get(message.guild.id, {})
    rules = guild_config.get("rules", "")
    
    if not rules:
        logging.warning(f"No rules found for {message.guild.name}, skipping AI check")
        return
    
    # Check for violations
    try:
        analysis = await ai_mod.check_message_violations(message, rules)
        
        if analysis and analysis.get("violates", False):
            # Handle the violation
            await ai_mod.handle_violation(
                message,
                analysis,
                None,  # warn_callback - not needed
                add_warning  # Pass the add_warning function
            )
            # FIX #3: Save to database after AI warning
            asyncio.create_task(batch_save_database())
    except Exception as e:
        logging.error(f"Error in AI message check: {e}")
